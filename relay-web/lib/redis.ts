...
export async function publishTaskUpdated(workspaceId: string, task: unknown) {
  await deliverToWS("task:updated", { workspaceId, task });
}
...
export async function publishTaskDeleted(
  ...
  await deliverToWS("task:deleted", { workspaceId, taskId, projectId });
}
